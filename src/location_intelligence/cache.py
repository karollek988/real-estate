"""Per-provider result cache (task F-06).
...
cadence — the docs/36 rule that ingestion cadence, not request cadence,
drives freshness. Stale entries are retained: on a failed refetch the
runner serves the stale copy, visibly marked (doc 37's stale-if-error).
...
class ProviderCache:
    ...
    def get(self, provider_id: str, key: str) -> CacheEntry | None:
        ...
        try:
            ...
            # A corrupt cache entry must never break a run — treat as a miss.
    ...
    def _path(self, provider_id: str, key: str) -> Path:
        # 16 hex chars (64 bits) keeps collisions negligible while staying
        # short — cache paths must survive Windows' 260-char MAX_PATH even
        # under deeply nested cache directories.
